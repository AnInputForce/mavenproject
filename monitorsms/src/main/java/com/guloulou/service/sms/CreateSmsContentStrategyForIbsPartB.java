 ...
 * @author Kang.Cunhua
 */
public class CreateSmsContentStrategyForIbsPartB extends CreateSmsContentStrategyAbst {

  public String editSmsContent() {
    System.out.println("调用策略类：" + "CreateSmsContentStrategyForIbsPartB!");
    ...
    String sql = "select  (1- ((select  count(asz0date)  from aspfz0 where asz0date=?  "
            ...
            + "where  asz0date=? ))*100  "
            + " transactionSuccessRate from dual";
    ...
    if (resultlist.size() > 0) {
      ...
      if (tmprate > Float.parseFloat(properties.getProperty("lowpercent"))
              && tmprate < Float.parseFloat(properties.getProperty("highpercent"))) {
        appendtmp.append(resultlist.get(0).getTransactionSuccessRate());
        String tempstr = appendtmp.toString();

        returntmp = updateTemplate(new IbsTransactionSuccessRate(Float.valueOf(tempstr)), properties.getProperty("ibsForPartB"));
      }
    }
    return returntmp;
  }
